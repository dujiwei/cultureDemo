...
@property (nonatomic, retain) UIImageView *calloutLeftCapView;
@property (nonatomic, retain) UIImageView *calloutRightCapView;
@property (nonatomic, retain) UIImageView *calloutAnchorView;
@property (nonatomic, retain) UIImageView *calloutLeftCenterView;
@property (nonatomic, retain) UIImageView *calloutRightCenterView;
...
- (void)setAnnotation:(NAAnnotation *)annotation{
    ...
    // 计算声音BUTTON
    if (annotation.rightCalloutAudioView) {
		middleWidth += annotation.rightCalloutAudioView.frame.size.width + NA_RIGHT_ACCESSORY_LEFT_OFFSET;
	}
    ...
    // 详情
    float accesoryWidth, accesoryWidth_x;
    if (annotation.rightCalloutAccessoryView) {
        accesoryWidth = annotation.rightCalloutAccessoryView.frame.size.width;
    ...
    float audioWidth;
    if (annotation.rightCalloutAudioView) {
        audioWidth = annotation.rightCalloutAudioView.frame.size.width;
        float x = leftCapWidth;
        
        CGRect frame = annotation.rightCalloutAccessoryView.frame;
        frame.origin.x = x-4;
        frame.origin.y = NA_RIGHT_ACCESSORY_TOP_OFFSET+1;
        annotation.rightCalloutAudioView.frame = frame;
        
        [self addSubview:annotation.rightCalloutAudioView];
        audioWidth += x;
	}
    ...
    if (annotation.subtitle) {
        ...
        self.subtitleLabel.frame = CGRectMake(audioWidth, NA_SUBTITLE_TOP_OFFSET, labelWidth, NA_SUBTITLE_LABEL_HEIGHT);
    ...
    self.titleLabel.frame = CGRectMake(audioWidth, titleTopOffset, labelWidth, titleLabelHeight);
...
- (void)dealloc {
    ...
    [_calloutLeftCapView release];
    [_calloutRightCapView release];
    [_calloutAnchorView release];
    [_calloutLeftCenterView release];
    [_calloutRightCenterView release];
